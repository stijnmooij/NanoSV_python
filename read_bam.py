...
group3.add_option('-r', '--matedistance', dest='matedistance', help='[i] Maximum distance to look for mateid. Default: 300')
...
group4.add_option('-i', '--pidf', dest='pidf', help='[s] Minimum median percentage identity to reference. Default: 0.80')
...
def parse_svs():
        ...
        for sv_min in sorted(structural_variants_region[segments[segment_id].rname]):
            ...
            for sv_max in sorted(structural_variants_region[segments[segment_id].rname][sv_min]):
                ...
                for sv_id in structural_variants_region[segments[segment_id].rname][sv_min][sv_max]:
                    ...
                    if sv_min > (segments[segment_id].pos + opts.refdistance) and sv_max < (segments[segment_id].end - opts.refdistance):
    ...
    for sv_id in sorted(structural_variants):
        ...
        for sv_id_2 in sorted(structural_variants_region_2[structural_variants[sv_id].chr]):
            ...
            if abs(structural_variants[sv_id].pos - structural_variants[sv_id_2].pos) <= opts.window and abs(structural_variants[sv_id].info['END'] - structural_variants[sv_id_2].info['END']) <= opts.window:
            ...
            if re.match("/\](\w+):(\d+)\]\w+/", structural_variants[sv_id].alt) and not re.match("/\w+\[(\w+):(\d+)\[/", structural_variants[sv_id_2].alt): continue
            if re.match("/\[(\w+):(\d+)\[\w+/", structural_variants[sv_id].alt) and not re.match("/\w+\](\w+):(\d+)\]/", structural_variants[sv_id_2].alt): continue
            if re.match("/\w+\](\w+):(\d+)\]/", structural_variants[sv_id].alt) and not re.match("/\[(\w+):(\d+)\[\w+/", structural_variants[sv_id_2].alt): continue
            if re.match("/\w+\[(\w+):(\d+)\[/", structural_variants[sv_id].alt) and not re.match("/\](\w+):(\d+)\]\w+/", structural_variants[sv_id_2].alt): continue
            if abs(structural_variants[sv_id].pos - structural_variants[sv_id_2].pos) > opts.matedistance: continue
            if abs(structural_variants[sv_id].info['END'] - structural_variants[sv_id_2].info['END']) > opts.matedistance: continue
        ...
        if structural_variants[sv_id].info['GAP'] > opts.gap and structural_variants[sv_id].info['SVTYPE'] != "INS": structural_variants[sv_id].filter.append("GAP")
        ...
        if structural_variants[sv_id].info['SVTYPE'] == 'INS': structural_variants[sv_id].info['SVLEN'] = structural_variants[sv_id].info['GAP']
        structural_variants[sv_id].printVCF()
        print(len(structural_variants), "leaving")
            ...
            for breakpoint_id in breakpoints_region_2[pos_2][pos_1]:
                ...
                if (prev_pos_2 == -1):
                    ...
                    if sum(sv.format['DV']) >= opts.count * 2:
                            ...
                            try:
                                for hanging_pos in sorted(hanging_breakpoints_region[sv.chr]['H']):
                                    ...
                                    for hanging_id in hanging_breakpoints_region[sv.chr]['H'][hanging_pos]:
                                        ...
                                        sv.addInfoField("PID", [[segments[segment_id].opts.pid], None])
                            ...
                            try:
                                for hanging_pos in sorted(hanging_breakpoints_region[sv.chr2]['T']):
                                    ...
                                    for hanging_id in hanging_breakpoints_region[sv.chr2]['T'][hanging_pos]:
                                        ...
                                        sv.format['HR'][1] += 1
                                        sv.format['VO'][0] += (1 - 10 ** (-segments[segment_id].mapq / 10.0))
                                        sv.addInfoField("PID", [None, [segments[segment_id].opts.pid]])
                        ...
                        print(len(structural_variants), "the script")
...
def parse_breakpoints():
    ...
    print(len(breakpoints_region), "tea")
...
def parse_reads():
    ...
    for qname in reads:
        ...
        for i in range(0, (len(clips)-1)):
            ...
            print(values, ", ", breakpoint.segment_1.pos, ", ", breakpoint.id, "coffee")
            print(i, segment_1.clip, "you get to hear me now")
            ...
            if i2 == len(clips) and segment_2.clip_2 >= opts.unmapped:
