...
def parse_svs():
                ...
                for sv_id in breakpoint.structural_variants_region[bam.segments[segment_id].rname][sv_min][sv_max]:
                    ...
                    if sv_min > (bam.segments[segment_id].pos + NanoSV.opts_refread_distance) and \
                        ...
                        if sv_id in svs_and_segments:
                            svs_and_segments[sv_id][x].append(bam.segments[segment_id])
                        else:
                            if x == 0:
                                svs_and_segments[sv_id] = [[bam.segments[segment_id]], []]
                            else:
                                svs_and_segments[sv_id] = [[], [bam.segments[segment_id]]]
    ...
    readname_sv_db = {}
    for sv_id in breakpoint.structural_variants:
        readname_sv_db[sv_id] = []
        for bp in breakpoint.structural_variants[sv_id].breakpoints:
            try:
                svs_and_segments[sv_id][0].append(bam.segments[read.breakpoints[bp].segment_1['id']])
                svs_and_segments[sv_id][1].append(bam.segments[read.breakpoints[bp].segment_2['id']])
                readname_sv_db[sv_id].append(bam.segments[read.breakpoints[bp].segment_1['id']].qname)
                readname_sv_db[sv_id].append(bam.segments[read.breakpoints[bp].segment_2['id']].qname)
            except KeyError:
                ""
    ...
    return [svs_and_segments, readname_sv_db]